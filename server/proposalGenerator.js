...
- Include an abstract of 90--130 words and a short keywords line of 4--6 terms before the numbered sections.
- Use this exact top-level section structure: Introduction; Project Goal; Methods; Expected Results and Research Milestones; Evaluation Plan; Risks and Mitigation; Resources, Tools, Budget, or Release Plan; References, Assumptions, or Source Notes.
- Use this section budget: Introduction 2--3 compact paragraphs; Project Goal 1 compact paragraph; Methods 2 compact paragraphs plus one figure; Expected Results and Research Milestones 1 paragraph plus 3--5 milestone bullets; Evaluation Plan 1--2 compact paragraphs; Risks and Mitigation 1 compact paragraph plus at most 3 bullets; Resources and source notes compact.
- Include one readable LaTeX-native figure or diagram in the Methods section with a caption, a label, and a text reference such as Figure~\\ref{fig:workflow}.
- Use minimal bullets, only for milestones, risk items, or source notes.
...
- Do not copy workflow todo text, critique instructions, missing-information lists, or revision planning notes into the proposal.
...
- Compare novelty against relevant prior work, examples, tools, workflows, or explicit source notes without inventing citations.
- Name concrete evaluation checks or metrics instead of vague promises to evaluate later.
...
export async function generateProposalBlueprint(payload) {
  ...
  return generateBlueprintWithApi(ideaInput, ideaPreview);
}
...
export async function generateRelatedWorkPlan(payload) {
  ...
  return generateRelatedWorkWithApi(ideaInput, proposalBlueprint);
}
...
export async function generateCritiquePanelResult(payload) {
  ...
  return generateCritiqueWithApi(ideaInput, proposalBlueprint, relatedWorkPlan);
}
...
export async function applyAcceptedRevisions(payload) {
  ...
  return generateRevisionResultWithApi(proposalBlueprint, revisionPlan);
}
...
async function refineProjectWithApi(payload) {
  ...
  assertObject(parsed?.project, 'project');
  assertArray(parsed?.fieldSuggestions, 'fieldSuggestions');
  assertArray(parsed?.decisions, 'decisions');
  assertArray(parsed?.questions, 'questions');
  assertArray(parsed?.updates, 'updates');
...
async function generateWithApi(project, checklist, proposalBlueprint) {
  ...
  const promptPayload = {
    ...
    styleTarget: {
      ...
      sectionOrder: [
        ...
        'Project Goal',
        'Methods',
        'Expected Results and Research Milestones',
        'Evaluation Plan',
        'Risks and Mitigation',
        'Resources, Tools, Budget, or Release Plan',
        'References, Assumptions, or Source Notes'
  ...
  try {
    ...
    if (!/invalid JSON|instead of the required strict JSON/i.test(error instanceof Error ? error.message : String(error))) {
      throw error;
    }
...
Your previous final-proposal response was invalid JSON. Retry once and return only a syntactically valid JSON object. Escape every LaTeX backslash and newline inside "proposalLatex" as required by JSON. Do not wrap the object in Markdown.`,
      payload: promptPayload,
      model,
      temperature: 0
    });
...
async function callGemini({ systemPrompt, payload, model, temperature }) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      return await callGeminiOnce({ systemPrompt, payload, model, temperature });
    } catch (error) {
      const status = Number(error?.status);
      const message = error instanceof Error ? error.message : String(error);
      lastError = error;

      if (attempt >= maxAttempts || !isTransientGeminiFailure(status, message)) {
        throw formatGeminiFailure(error, attempt);
      }

      await waitForRetry(calculateGeminiRetryDelayMs(error, attempt));
    }
  }

  throw formatGeminiFailure(lastError, maxAttempts);
}

async function callGeminiOnce({ systemPrompt, payload, model, temperature }) {
  ...
  if (!response.ok) {
    const error = new Error(data?.error?.message || `Gemini API returned ${response.status}`);
    error.status = response.status;
    error.retryDelayMs = extractGeminiRetryDelayMs(data?.error);
    throw error;
  }
...
async function waitForRetry(ms) {
  const scale = Number(process.env.LLM_RETRY_DELAY_SCALE ?? 1);
  const scaledMs = Number.isFinite(scale) ? Math.max(0, ms * scale) : ms;
...
function calculateGeminiRetryDelayMs(error, attempt) {
  const retryDelayMs = Number(error?.retryDelayMs);
  const fallbackMs = 1200 * attempt;
  const selectedMs = Number.isFinite(retryDelayMs) && retryDelayMs > 0 ? retryDelayMs : fallbackMs;

  return Math.min(selectedMs, 30_000);
}

function extractGeminiRetryDelayMs(error) {
  const details = Array.isArray(error?.details) ? error.details : [];

  for (const detail of details) {
    const retryDelay = clean(detail?.retryDelay);
    const parsedMs = parseDurationMs(retryDelay);
  ...
  const messageDelay = clean(error?.message).match(/retry in\s+([0-9]+(?:\.[0-9]+)?)s/i);
...
function parseDurationMs(value) {
  const match = clean(value).match(/^([0-9]+(?:\.[0-9]+)?)s$/i);
...
function formatGeminiFailure(error, attempts) {
  const status = Number(error?.status);
  const message = error instanceof Error ? error.message : clean(error);
  const isQuota = status === 429 || /quota|resource exhausted|rate limit/i.test(message);

  if (isQuota) {
    return new Error(`Gemini API quota exceeded after ${attempts} attempt(s). Try again later or increase quota/billing. ${message}`);
  }

  if (isTransientGeminiFailure(status, message)) {
    return new Error(`Gemini API request failed after ${attempts} attempt(s). ${message}`);
  }

  return error instanceof Error ? error : new Error(message || 'Gemini API request failed.');
}
...
function isTransientGeminiFailure(status, message) {
  ...
  return (
    ...
    status === 500 ||
    status === 503 ||
    /currently experiencing high demand|temporarily unavailable|try again later|resource exhausted/i.test(normalized)
  );
}
...
function requireApiConfig() {
  ...
  if (missing.length) {
    throw new Error(`API-backed proposal generation is required. Configure ${missing.join(', ')} in .env and restart the server.`);
  }

  return {
    apiKey: clean(process.env.LLM_API_KEY),
    apiUrl: clean(process.env.LLM_API_URL),
...
function buildLocalProposalBlueprint(ideaInput, ideaPreview) {
  const workingTitle = titleCase(ideaPreview.projectTitle || ideaInput.topic || 'Research Proposal Blueprint');
  ...
  const dataSystems = clean(ideaInput.datasets) || clean(ideaPreview.resources) || 'Candidate datasets, tools, and systems still need to be selected.';
  const missingInformation = buildBlueprintMissingInformation(ideaInput, ideaPreview);
  const evaluationPlan = clean(ideaPreview.evaluationPlan) || buildEvaluationPlan(ideaInput);
...
function buildLocalProposalLatex(project, proposalBlueprint) {
  ...
  const sourceNotes = dedupeStrings([
    ...missingInformation,
    ...buildSourceNoteItems(project.references),
    'Novelty claims remain provisional until the related literature and comparison tools are verified.'
  ].filter(Boolean));
  const keywords = buildProposalKeywords(project, proposalBlueprint);
  ...
  const introduction = mergeProposalParagraphs([
    ...
    buildPriorWorkSentence(project, sourceNotes)
  ]);
  const figureReferenceSentence = 'Figure~\\ref{fig:workflow} summarizes the planned workflow, including input collection, analysis, modeling or drafting, quality checks, and revision decisions.';
  const methods = mergeProposalParagraphs([
    sanitizeProposalText(proposalBlueprint?.proposedMethod)
  ...
  const expectedResults = mergeProposalParagraphs([
    ...
    sanitizeProposalText(project.timeline),
    buildMilestoneSentence(project),
    sanitizeProposalText(proposalBlueprint?.intellectualMerit),
    sanitizeProposalText(proposalBlueprint?.broaderImpacts)
  ]);
  const evaluationPlan = mergeProposalParagraphs([
    ...
    buildConcreteEvaluationSentence(project, proposalBlueprint)
  ]);
  const riskItems = buildProposalRiskItems(proposalBlueprint, project).slice(0, 3);
  const risks = mergeProposalParagraphs([
    'Key risks include weak novelty support, incomplete source verification, narrow evaluation coverage, and timelines that assume ideal implementation progress.',
    'Mitigation will focus on labeling unsupported claims as assumptions, narrowing the project scope when needed, and using revision checkpoints before final submission.'
  ]);
  const resources = mergeProposalParagraphs([
    sanitizeProposalText(project.resources)
      || sanitizeProposalText(proposalBlueprint?.datasetsToolsSystems)
      || 'The project will rely on the most relevant publicly available datasets, tools, or source materials that fit the chosen domain.',
    'The release plan is to keep proposal source, figure source, source notes, and workflow evidence in the course repo so the final PDF can be reproduced.'
  ]);
  const abstract = mergeProposalParagraphs([
    sanitizeProposalText(proposalBlueprint?.oneSentenceSummary)
      || `This proposal studies ${sanitizeProposalText(project.topic || title)}.`,
    sanitizeProposalText(proposalBlueprint?.problemStatement) || sanitizeProposalText(project.problem),
    sanitizeProposalText(proposalBlueprint?.proposedMethod) || sanitizeProposalText(project.method),
    buildAbstractEvaluationSentence(project, proposalBlueprint)
  ]);
...
${latexParagraph(introduction)}

\section{Project Goal}
${latexParagraph(projectGoal)}

\section{Methods}
${latexParagraph(methods)}
${figureReferenceSentence}
...
Idea intake or dataset collection $\rightarrow$ structured analysis and feature design $\rightarrow$ model or draft generation $\rightarrow$ proposal quality checks $\rightarrow$ targeted revision loop.
}}
\caption{Planned workflow for the proposed study and its revision loop.}
\label{fig:workflow}
\end{figure}

\section{Expected Results and Research Milestones}
${latexParagraph(expectedResults)}

\section{Evaluation Plan}
${latexParagraph(evaluationPlan)}
...
function buildProposalKeywords(project, proposalBlueprint) {
  const topicTerms = clean(project.topic || project.title || proposalBlueprint?.workingTitle)
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((term) => term.length > 3);
  const candidateKeywords = dedupeStrings([
    ...topicTerms.slice(0, 2),
    'research proposal',
    'agent workflow',
    'evaluation plan',
    'revision loop'
  ...
  return candidateKeywords.slice(0, 6).join(', ');
}

function buildSourceNoteItems(value) {
  return clean(value)
    .split(/\n|;/)
...
function buildPriorWorkSentence(project, sourceNotes) {
  if (sourceNotes.length) {
    return `The proposal will compare its novelty against related prior work, source notes, and existing tools listed in the references section, while keeping unsupported claims provisional.`;
  }

  if (clean(project.references)) {
    return 'The proposal will compare its novelty against the related prior work and tools named in the current source notes.';
  }

  return 'The proposal will compare its novelty against relevant prior work, examples, or tools after the literature review is verified.';
}

function buildMilestoneSentence(project) {
  if (clean(project.timeline)) {
    return 'The timeline above is treated as a research milestone plan rather than a promise of immediate implementation success.';
  }

  return 'A simple milestone plan is to complete prior-work review first, finish workflow design next, run evaluation after that, and use the last phase for revision and source-note cleanup.';
}

function buildConcreteEvaluationSentence(project, proposalBlueprint) {
  const evaluation = clean(proposalBlueprint?.evaluationPlan || project.evaluation);

  if (hasConcreteEvaluationSignals(evaluation)) {
    return '';
  }

  return 'Concrete checks should include required-section coverage, unresolved-assumption count, baseline comparison, and at least one readability or reviewer-judgment score.';
}

function buildAbstractEvaluationSentence(project, proposalBlueprint) {
  const evaluation = clean(proposalBlueprint?.evaluationPlan || project.evaluation);

  if (hasConcreteEvaluationSignals(evaluation)) {
    return `The plan will test the proposal with concrete metrics and checks: ${evaluation}`;
  }

  return 'The plan will test proposal quality with concrete metrics such as section coverage, unresolved assumptions, baseline comparison, and reviewer judgment.';
}
...
function buildQuestionObjects(project) {
  ...
  if (!isSpecific(project.resources, 30)) {
    add(
      'resources',
      'What tools, APIs, files, or provider configuration will make this reproducible?',
...
function buildFieldSuggestions(project) {
  ...
  const suggestions = [
    ...
    },
    {
      field: 'resources',
      label: 'Resources',
    value: project.resources || 'React, Vite, Node, Gemini API, sample research ideas, and course requirements.',
...
function normalizeIdeaPreviewPayload(payload) {
  return {
    ...
    projectTitle: clean(payload.projectTitle),
    evaluationPlan: clean(payload.evaluationPlan),
...
function buildSearchQueries(ideaInput, proposalBlueprint, keyConcepts) {
  const topic = compactQueryFragment(clean(ideaInput.topic) || clean(proposalBlueprint.workingTitle), 'research proposal workflow');
  const method = compactQueryFragment(clean(ideaInput.methods) || clean(proposalBlueprint.proposedMethod), 'method');
  const evaluation = compactQueryFragment(clean(proposalBlueprint.evaluationPlan), 'evaluation');
  const domain = compactQueryFragment(clean(ideaInput.domain), 'research');
  const gapFocus = compactQueryFragment(findGapFocus(proposalBlueprint.researchGap), 'gap');
  const contribution = compactQueryFragment(clean(proposalBlueprint.proposedContribution), 'contribution');
  const conceptA = compactQueryFragment(keyConcepts[0], topic);
  const conceptB = compactQueryFragment(keyConcepts[1], domain || 'evaluation');
  const conceptC = compactQueryFragment(keyConcepts[2], 'literature review');
  ...
  const queries = [
    ...
    joinQueryParts([domain, contribution, 'benchmark']),
...
function compactQueryFragment(value, fallback = '') {
  ...
  const compact = dedupeStrings(tokens).slice(0, 6).join(' ');
  return compact || clean(fallback);
}
...
function buildCriticReview(criticName, criticRole, score, summary, strengths, issues, overallRecommendation) {
  return {
    ...
    issues: sortCritiqueIssues(issues),
    overallRecommendation
  };
}
...
function buildComplianceMatrix({ checklist, proposalLatex, project, modelMatrix = [] }) {
  const artifact = {
    title: extractLatexTitle(proposalLatex),
    abstract: extractLatexAbstract(proposalLatex),
    keywords: extractLatexKeywords(proposalLatex),
    figure: extractFigureArtifact(proposalLatex),
    sections: extractProposalSections(proposalLatex)
  };
  const modelRows = new Map(
    modelMatrix
      ...
      .map((row) => [normalizeRequirementKey(row.requirement), row])
  );

  return checklist.map((requirement) => {
    const modelRow = modelRows.get(normalizeRequirementKey(requirement));
    const evidence = findRequirementEvidence(requirement, artifact, project);
    const covered = Boolean(evidence);

    return {
      requirement,
      status: covered ? 'Covered' : 'Needs work',
      evidence: evidence || modelRow?.evidence || 'The generated proposal artifact does not clearly cover this requirement yet.',
      fix: covered ? modelRow?.fix || 'Keep this section specific.' : modelRow?.fix || `Revise the generated proposal to explicitly cover: ${requirement}.`
    };
  });
}

function findRequirementEvidence(requirement, artifact, project) {
  const text = normalizeRequirementKey(requirement);
  const sections = artifact.sections;

  if (/title/.test(text) && artifact.title) return artifact.title;
  if (/abstract/.test(text)) return summarizeArtifactEvidence(artifact.abstract);
  if (/keyword/.test(text)) return summarizeArtifactEvidence(artifact.keywords);
  if (/introduction|motivation|problem|gap|prior work/.test(text)) {
    const introduction = sections.get('introduction');
    if (introduction && hasPriorWorkGrounding(introduction)) return summarizeArtifactEvidence(introduction);
    return '';
  }
  if (/goal/.test(text)) return summarizeArtifactEvidence(sections.get('project goal')) || fallbackRequirementEvidence(text, project);
  if (/method|workflow|approach/.test(text)) {
    return summarizeArtifactEvidence(sections.get('methods')) || fallbackRequirementEvidence(text, project);
  }
  if (/figure|diagram|caption/.test(text)) {
    if (artifact.figure.hasFigure && artifact.figure.caption && artifact.figure.referenced) {
      return summarizeArtifactEvidence(`${artifact.figure.caption} Referenced in text with ${artifact.figure.referenceLabel || 'a figure reference'}.`);
    }

    return '';
  }
  if (/expected|result|milestone|timeline/.test(text)) {
    return summarizeArtifactEvidence(sections.get('expected results and research milestones')) || fallbackRequirementEvidence(text, project);
  }
  if (/evaluation|metric|test/.test(text)) {
    const evaluation = sections.get('evaluation plan');
    if (evaluation && hasConcreteEvaluationSignals(evaluation)) return summarizeArtifactEvidence(evaluation);
    return '';
  }
  if (/risk|mitigation/.test(text)) {
    return summarizeArtifactEvidence(sections.get('risks and mitigation')) || fallbackRequirementEvidence(text, project);
  }
  if (/resource|budget|tool|release/.test(text)) {
    return summarizeArtifactEvidence(sections.get('resources tools budget or release plan')) || fallbackRequirementEvidence(text, project);
  }
  if (/reference|assumption|source/.test(text)) {
    return summarizeArtifactEvidence(sections.get('references assumptions or source notes')) || fallbackRequirementEvidence(text, project);
  }

  return fallbackRequirementEvidence(text, project);
}

function fallbackRequirementEvidence(requirementText, project) {
  if (/title/.test(requirementText) && project.title) return project.title;
  if (/abstract/.test(requirementText) && (project.problem || project.method)) return 'Draft abstract can be generated from the current project summary and method.';
  if (/keyword/.test(requirementText) && (project.topic || project.title)) return buildProposalKeywords(project, {});
  if (/introduction|motivation|gap|problem|prior work/.test(requirementText) && project.problem) return project.problem;
  if (/goal/.test(requirementText) && project.title) return 'Goal section is generated from the current proposal direction and contribution.';
  if (/method|workflow|approach/.test(requirementText) && project.method) return project.method;
  if (/figure|diagram|caption/.test(requirementText) && project.method) return 'A workflow figure can be derived from the planned method once the final proposal is drafted.';
  if (/expected|result|milestone|timeline/.test(requirementText) && project.timeline) return project.timeline;
  if (/evaluation|metric|test/.test(requirementText) && project.evaluation) return project.evaluation;
  if (/risk|mitigation/.test(requirementText) && (project.timeline || project.problem)) {
    return 'Draft includes risk and mitigation notes tied to scope, evidence, and schedule.';
  }
  if (/resource|budget|tool|release/.test(requirementText) && project.resources) return project.resources;
  if (/expected|contribution|result/.test(requirementText) && (project.timeline || project.evaluation || project.problem)) {
    return project.timeline || 'Draft includes expected results and milestone notes.';
  }
  if (/reference|assumption|source/.test(requirementText) && project.references) return project.references;

  return '';
}

function extractLatexTitle(proposalLatex) {
  const match = String(proposalLatex || '').match(/\\title\{([\s\S]*?)\}/);
...
function extractLatexAbstract(proposalLatex) {
  const match = String(proposalLatex || '').match(/\\begin\{abstract\}([\s\S]*?)\\end\{abstract\}/i);
  return cleanLatexSectionBody(match?.[1] || '');
}

function extractLatexKeywords(proposalLatex) {
  const match = String(proposalLatex || '').match(/\\textbf\{Keywords:\}\s*([^\n]+)/i);
  return cleanLatexSectionBody(match?.[1] || '');
}

function extractFigureArtifact(proposalLatex) {
  const source = String(proposalLatex || '');
  const figureMatch = source.match(/\\begin\{figure\}[\s\S]*?\\caption\{([\s\S]*?)\}[\s\S]*?\\label\{([^}]*)\}[\s\S]*?\\end\{figure\}/i);
  const referenceLabel = clean(figureMatch?.[2]);

  return {
    hasFigure: Boolean(figureMatch),
    caption: cleanLatexSectionBody(figureMatch?.[1] || ''),
    referenceLabel,
    referenced: referenceLabel ? new RegExp(`(?:Figure|Fig\\.)\\s*~?\\\\ref\\{${escapeRegExp(referenceLabel)}\\}`, 'i').test(source) : false
  };
}

function extractProposalSections(proposalLatex) {
  const source = String(proposalLatex || '');
  ...
  matches.forEach((match, index) => {
    ...
    sections.set(normalizeRequirementKey(match[1]), cleanLatexSectionBody(source.slice(start, end)));
  });

  return sections;
}

function cleanLatexSectionBody(value) {
  return String(value || '')
...
function summarizeArtifactEvidence(value) {
  const cleaned = clean(value);
  if (!cleaned) return '';
  return cleaned.length > 180 ? `${cleaned.slice(0, 177).trim()}...` : cleaned;
}

function hasPriorWorkGrounding(value) {
  const text = normalizeRequirementKey(value);
  return /(prior work|related work|source note|source notes|compare|comparison|workflow|tool|literature|baseline)/.test(text);
}

function hasConcreteEvaluationSignals(value) {
  const text = normalizeRequirementKey(value);
  return /(metric|metrics|accuracy|precision|recall|f1|auc|mae|rmse|coverage|baseline|before after|reviewer|rubric|score|scores|ablation|false positive|false negatives|test scenario|pass fail|latency)/.test(text);
}
...
function parseJsonContent(content) {
  ...
  try {
    ...
    if (looksLikeLatex(trimmed)) {
      throw new Error('The API returned LaTeX text instead of the required strict JSON object.');
    }

    throw new Error(`The API returned invalid JSON. ${error instanceof Error ? error.message : String(error)}`);
  }
}

function coerceResult(result, project, checklist, proposalBlueprint) {
  const proposalLatex = extractProposalLatex(result, project, proposalBlueprint);
  assertNonEmptyString(proposalLatex, 'proposalLatex');
  assertArray(result?.complianceMatrix, 'complianceMatrix');
  assertNonEmptyString(result?.evaluationReport, 'evaluationReport');
  assertArray(result?.questions, 'questions');
  assertSubmissionReadyProposal(proposalLatex, project);

  return {
    proposalLatex,
    complianceMatrix: buildComplianceMatrix({
      checklist,
      proposalLatex,
      project,
      modelMatrix: Array.isArray(result.complianceMatrix) ? result.complianceMatrix : []
    }),
...
function coerceBlueprintResult(result, ideaInput, ideaPreview) {
  return {
    workingTitle: readRequiredString(result, ['workingTitle', 'working_title'], 'workingTitle'),
    oneSentenceSummary: readRequiredString(result, ['oneSentenceSummary', 'one_sentence_summary'], 'oneSentenceSummary'),
    problemStatement: readRequiredString(result, ['problemStatement', 'problem_statement'], 'problemStatement'),
    motivation: readRequiredString(result, ['motivation'], 'motivation'),
    researchGap: readRequiredString(result, ['researchGap', 'research_gap'], 'researchGap'),
    proposedContribution: readRequiredString(result, ['proposedContribution', 'proposed_contribution'], 'proposedContribution'),
    researchQuestions: readRequiredStringArray(result, ['researchQuestions', 'research_questions'], 'researchQuestions'),
    hypotheses: readRequiredStringArray(result, ['hypotheses'], 'hypotheses'),
    proposedMethod: readRequiredString(result, ['proposedMethod', 'proposed_method'], 'proposedMethod'),
    datasetsToolsSystems: readRequiredString(result, ['datasetsToolsSystems', 'datasets_tools_systems', 'datasetsTools'], 'datasetsToolsSystems'),
    evaluationPlan: readRequiredString(result, ['evaluationPlan', 'evaluation_plan'], 'evaluationPlan'),
    expectedResults: readRequiredString(result, ['expectedResults', 'expected_results'], 'expectedResults'),
    intellectualMerit: readRequiredString(result, ['intellectualMerit', 'intellectual_merit'], 'intellectualMerit'),
    broaderImpacts: readRequiredString(result, ['broaderImpacts', 'broader_impacts'], 'broaderImpacts'),
    missingInformation: readRequiredStringArray(result, ['missingInformation', 'missing_information'], 'missingInformation'),
    suggestedNextSteps: readRequiredStringArray(result, ['suggestedNextSteps', 'suggested_next_steps'], 'suggestedNextSteps')
  };
}

function coerceRelatedWorkResult(result, ideaInput, proposalBlueprint) {
  return {
    searchQueries: readRequiredStringArray(result, ['searchQueries', 'search_queries'], 'searchQueries'),
    keyConcepts: readRequiredStringArray(result, ['keyConcepts', 'key_concepts'], 'keyConcepts'),
    relatedWorkBuckets: readRequiredBucketArray(result, ['relatedWorkBuckets', 'related_work_buckets'], 'relatedWorkBuckets'),
    suggestedVenuesOrSources: readRequiredStringArray(
      result,
      ['suggestedVenuesOrSources', 'suggested_venues_or_sources'],
      'suggestedVenuesOrSources'
    ),
    literatureGapQuestions: readRequiredStringArray(
      result,
      ['literatureGapQuestions', 'literature_gap_questions'],
      'literatureGapQuestions'
    ),
    unsupportedClaimWarnings: readRequiredStringArray(
      result,
      ['unsupportedClaimWarnings', 'unsupported_claim_warnings'],
      'unsupportedClaimWarnings'
    ),
    nextSteps: readRequiredStringArray(result, ['nextSteps', 'suggestedNextSteps', 'next_steps'], 'nextSteps')
  };
}

function coerceCritiquePanelResult(result, ideaInput, proposalBlueprint, relatedWorkPlan) {
  const reviews = readRequiredReviewArray(result, ['reviews'], 'reviews');
  ...
  return {
    overallScore: readRequiredNumber(result, ['overallScore', 'overall_score'], 'overallScore'),
    reviews,
    highestPriorityIssues,
    suggestedRevisionOrder: readRequiredStringArray(
      result,
...
function coerceApplyRevisionResult(result, proposalBlueprint, revisionPlan) {
  return {
    ...
    changeSummary: readRequiredStringArray(result, ['changeSummary', 'change_summary'], 'changeSummary'),
    changedSections: readRequiredChangedSectionArray(
      result,
...
function extractProposalLatex(result) {
  const candidates = [
  ...
  throw new Error('The API response is missing a complete compile-ready proposalLatex document.');
}
...
function matchesClassStyleProposal(value) {
  ...
  return (
    ...
    latex.includes('\\begin{figure}') &&
    /(?:Figure|Fig\.)\s*~?\\ref\{fig:workflow\}/i.test(latex) &&
...
function assertSubmissionReadyProposal(proposalLatex, project = {}) {
  const failures = validateSubmissionReadyProposal(proposalLatex, project);

  if (failures.length) {
    throw new Error(`Generated proposal failed submission checks: ${failures.join('; ')}.`);
  }
}

function validateSubmissionReadyProposal(proposalLatex, project = {}) {
  const latex = clean(proposalLatex);
  const failures = [];
  const sections = extractProposalSections(latex);
  const abstract = extractLatexAbstract(latex);
  const abstractWordCount = countWords(abstract);
  const keywords = extractLatexKeywords(latex);
  const keywordCount = keywords
    .split(/[,;]+/)
    .map(clean)
    .filter(Boolean).length;
  const figure = extractFigureArtifact(latex);
  const pageCount = estimateProposalPageCount(latex, project.title || project.topic || 'Research Proposal');
  const evaluation = sections.get('evaluation plan') || '';
  const references = sections.get('references assumptions or source notes') || '';
  const introduction = sections.get('introduction') || '';
  const milestones = sections.get('expected results and research milestones') || '';
  const allText = cleanLatexSectionBody(latex);

  if (!/\\documentclass\[[^\]]*\b11pt\b[^\]]*\]\{article\}/.test(latex)) {
    failures.push('document must use 11pt article class');
  }

  if (!hasOneInchMargins(latex)) {
    failures.push('document must use 1-inch margins');
  }

  if (pageCount > 3) {
    failures.push(`PDF is ${pageCount} pages`);
  }

  if (abstractWordCount < 90 || abstractWordCount > 130) {
    failures.push(`abstract must be 90-130 words, found ${abstractWordCount}`);
  }

  if (keywordCount < 4 || keywordCount > 6) {
    failures.push(`keywords must contain 4-6 terms, found ${keywordCount}`);
  }
  ...
  requiredSectionLabels.forEach((label) => {
    if (!sections.has(label)) {
      failures.push(`missing required section: ${titleCase(label)}`);
    }
  });

  if (!figure.hasFigure || !figure.caption || !figure.referenced) {
    failures.push('figure must include a caption, label, and text reference');
  }

  if (!hasPriorWorkGrounding(introduction)) {
    failures.push('introduction must compare against prior work, tools, workflows, or source notes');
  }

  if (!hasConcreteEvaluationSignals(evaluation) || countMetricSignals(evaluation) < 2) {
    failures.push('evaluation plan must name concrete metrics or tests');
  }

  if (!/\b(source|sources|assumption|assumptions|reference|references|prior work|literature|et al|http)/i.test(references)) {
    failures.push('references/source-notes section must include source notes or assumptions');
  }

  if (!/(week|month|phase|milestone|timeline|weeks?|months?|phases?)/i.test(milestones)) {
    failures.push('milestones section must include timeline estimates');
  }
  ...
  const bannedPatterns = [
    ...
    /fallback mode/i,
    /template mode/i,
    /review the revised blueprint/i,
    /resolve the missing-information list/i,
    /missing-information list/i,
    /next workflow stage/i,
    /run a critique pass/i,
    /treating this blueprint as a stable proposal version/i
  ];
  const bannedMatch = bannedPatterns.find((pattern) => pattern.test(allText));

  if (bannedMatch) {
    failures.push(`proposal contains workflow/debug artifact matching ${bannedMatch.source}`);
  }

  if (hasRepeatedLongText(allText)) {
    failures.push('proposal repeats long blocks of text');
  }

  return failures;
}

function hasOneInchMargins(latex) {
  return (
    /\\usepackage\[[^\]]*\bmargin\s*=\s*1in\b[^\]]*\]\{geometry\}/i.test(latex) ||
    /\\geometry\{[^}]*\bmargin\s*=\s*1in\b[^}]*\}/i.test(latex)
  );
}
...
function countMetricSignals(value) {
  const text = normalizeRequirementKey(value);
  const signals = [
    'accuracy',
    'precision',
...
function hasRepeatedLongText(value) {
  ...
  for (const sentence of sentences) {
    ...
    if (sentenceSeen.has(key)) return true;
...
function readRequiredString(result, keys, label) {
  const value = readFirstString(result, keys);
...
function assertNonEmptyString(value, label) {
  if (!clean(value)) {
    throw new Error(`The API response is missing required field: ${label}.`);
  }
}

function assertArray(value, label) {
  if (!Array.isArray(value)) {
    throw new Error(`The API response field ${label} must be an array.`);
  }
}

function assertObject(value, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`The API response field ${label} must be an object.`);
  }
}

function readRequiredStringArray(result, keys, label) {
  for (const key of keys) {
  ...
  throw new Error(`The API response is missing required array field: ${label}.`);
}

function readRequiredBucketArray(result, keys, label) {
  for (const key of keys) {
    const value = result?.[key];
    ...
    const buckets = value
      .map((entry, index) => {
        ...
        const whyItMatters = clean(entry?.whyItMatters || entry?.why_it_matters);
        const exampleSearchTermsValue = entry?.exampleSearchTerms || entry?.example_search_terms;
        const exampleSearchTerms = Array.isArray(exampleSearchTermsValue)
          ? exampleSearchTermsValue.map(clean).filter(Boolean)
          : typeof exampleSearchTermsValue === 'string'
            ? [clean(exampleSearchTermsValue)].filter(Boolean)
            : [];

        if (!title || !description) {
          throw new Error(`The API response field ${label}[${index}] must include title and description.`);
        }
...
function readRequiredChangedSectionArray(result, keys, label) {
  for (const key of keys) {
    const value = result?.[key];
    ...
    const sections = value
      .map((entry, index) => {
        const sectionName = clean(entry?.sectionName || entry?.section_name);
        const beforeSummary = clean(entry?.beforeSummary || entry?.before_summary);
        const afterSummary = clean(entry?.afterSummary || entry?.after_summary);
        const reasonForChange = clean(entry?.reasonForChange || entry?.reason_for_change);

        if (!sectionName || !reasonForChange || (!beforeSummary && !afterSummary)) {
          throw new Error(`The API response field ${label}[${index}] must include sectionName, summary text, and reasonForChange.`);
        }
...
function readRequiredReviewArray(result, keys, label) {
  for (const key of keys) {
    const value = result?.[key];
    ...
    const reviews = value
      .map((entry, index) => {
        const review = normalizeCriticReview(entry);

        if (!review?.criticName || !review?.summary || !review.issues.length) {
          throw new Error(`The API response field ${label}[${index}] must include criticName, summary, and issues.`);
        }
...
function readRequiredNumber(result, keys, label) {
  for (const key of keys) {
  ...
  throw new Error(`The API response is missing required numeric field: ${label}.`);
}
...
export const __testables = {
  ...
  buildSearchQueries,
  calculateGeminiRetryDelayMs,
  extractChecklist,
  extractFigureArtifact,
  extractGeminiRetryDelayMs,
  extractLatexAbstract,
  extractLatexKeywords,
  extractLatexTitle,
  extractProposalSections,
  hasConcreteEvaluationSignals,
  hasPriorWorkGrounding,
  normalizeCritiqueIssue,
  validateSubmissionReadyProposal
};
