...
- simulated Gemini-style pipeline outputs generated by Codex
- local Node scripts, tests, build, PDF rendering, and proposal validation

## Why The Run Is Simulated

A fresh Gemini API run could not be completed because the free-tier request quota was exhausted and no additional class credits were available. The instructor allowed simulated AI-agent output showing what the pipeline should have produced if credits were available.

The simulated run is labeled consistently in the evidence files:

- `mode: simulated-api-output`
- `provider: codex-simulated-gemini`

This is not represented as a live Gemini run.
...
- `npm run generate:simulated-submission`
- `npm test`
- `npm run build`
